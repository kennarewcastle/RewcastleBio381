...
### Batch Processing Simulated Data Files
...
### Breaking Batch Processing with Insufficient Data
...
The trick here is breaking the batch processing `for loop` and *not* the `FileBuilder` step. A good rule of thumb to prevent this is keeping the minimum number of rows created in the file well above the number of NA replacements for each file, else the sampling of data rows during NA replacement will throw an error message that halts file creation.

### Modifying Summary Output to List Sample Size

Additional data can be pulled from each file and listed in the `StatsSummary.csv` output by creating a column for this information to go during the building of the `statsOut` data frame skeleton. Then, extra lines of code can be added to the `for loop` that operates on each individual file. These lines of code inserts the number of rows in the original and clean data sets in column 6 and 7 of the output table using subsetting and the `nrow` function.
